...
Only choose from the list of cuisines and difficulties provided. Do not add any additional text or specific desription of the cuisine. The response should be a single word from the list.
...
def process_recipe(row: pd.Series) -> pd.Series:
    ...
    try:
        ...
        ingredient_names = [item["name"] for item in ingredients_list if isinstance(item, dict) and "name" in item]
...
def main():
    ...
    df.to_csv('data/recipe_200_processed.csv', index=False)
    print("Done!")
